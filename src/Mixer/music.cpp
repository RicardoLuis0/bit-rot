/*
  SDL_mixer:  An audio mixer library based on the SDL library
  Copyright (C) 1997-2024 Sam Lantinga <slouken@libsdl.org>

  This software is provided 'as-is', without any express or implied
  warranty.  In no event will the authors be held liable for any damages
  arising from the use of this software.

  Permission is granted to anyone to use this software for any purpose,
  including commercial applications, and to alter it and redistribute it
  freely, subject to the following restrictions:

  1. The origin of this software must not be misrepresented; you must not
     claim that you wrote the original software. If you use this software
     in a product, an acknowledgment in the product documentation would be
     appreciated but is not required.
  2. Altered source versions must be plainly marked as such, and must not be
     misrepresented as being the original software.
  3. This notice may not be removed or altered from any source distribution.
*/
#include "SDL2/SDL_hints.h"
#include "SDL2/SDL_log.h"
#include "SDL2/SDL_timer.h"

#include "SDL_mixer.h"
#include "mixer.h"
#include "music.h"

#include "codecs/music_wav.h"
#include "codecs/music_ogg.h"

#include "utils.h"

/* Check to make sure we are building with a new enough SDL */
#if SDL_COMPILEDVERSION < SDL_VERSIONNUM(2, 0, 7)
#error You need SDL 2.0.7 or newer from http://www.libsdl.org
#endif

/* Set this hint to true if you want verbose logging of music interfaces */
#define SDL_MIXER_HINT_DEBUG_MUSIC_INTERFACES \
    "SDL_MIXER_DEBUG_MUSIC_INTERFACES"

static bool music_active = SDL_TRUE;
static int music_volume = MIX_MAX_VOLUME;
//static Mix_Music * volatile music_playing = NULL;
SDL_AudioSpec music_spec;

typedef struct Mix_Music {
    Mix_MusicInterface *interface;
    void *context;

    bool playing;
    bool paused;
    Mix_Fading fading;
    int fade_step;
    int fade_steps;

    char filename[1024];
} Mix_Music;

typedef struct music_channel
{
    Mix_Music * volatile music_playing; // = NULL;
    int channel_volume; // = MIX_MAX_VOLUME;
} music_channel_t;

// only 2 channels for now, for cross-fading
#define NUM_MUSIC_CHANNELS 2

static music_channel_t music_channels[NUM_MUSIC_CHANNELS];

/* Used to calculate fading steps */
static int ms_per_step;

/* rcg06042009 report available decoders at runtime. */
static const char **music_decoders = NULL;
static int num_decoders = 0;

/* Meta-Tags utility */
extern "C" void meta_tags_init(Mix_MusicMetaTags *tags)
{
    SDL_memset(tags, 0, sizeof(Mix_MusicMetaTags));
}

extern "C" void meta_tags_clear(Mix_MusicMetaTags *tags)
{
    size_t i;

    for (i = 0; i < MIX_META_LAST; i++) {
        if (tags->tags[i]) {
            SDL_free(tags->tags[i]);
            tags->tags[i] = NULL;
        }
    }
}

extern "C" void meta_tags_set(Mix_MusicMetaTags *tags, Mix_MusicMetaTag type, const char *value)
{
    char *out;
    size_t len;

    if (!value) {
        return;
    }
    if (type >= MIX_META_LAST) {
        return;
    }

    len = SDL_strlen(value);
    out = (char *)SDL_malloc(sizeof(char) * len + 1);
    SDL_strlcpy(out, value, len +1);

    if (tags->tags[type]) {
        SDL_free(tags->tags[type]);
    }

    tags->tags[type] = out;
}

extern "C" const char *meta_tags_get(Mix_MusicMetaTags *tags, Mix_MusicMetaTag type)
{
    switch (type) {
    case MIX_META_TITLE:
    case MIX_META_ARTIST:
    case MIX_META_ALBUM:
    case MIX_META_COPYRIGHT:
        return tags->tags[type] ? tags->tags[type] : "";
    case MIX_META_LAST:
    default:
        break;
    }
    return "";
}

/* for music->filename */
#if defined(_WIN32) || defined(__OS2__)
static SDL_INLINE const char *get_last_dirsep (const char *p) {
    const char *p1 = SDL_strrchr(p, '/');
    const char *p2 = SDL_strrchr(p, '\\');
    if (!p1) return p2;
    if (!p2) return p1;
    return (p1 > p2)? p1 : p2;
}
#else /* unix */
static SDL_INLINE const char *get_last_dirsep (const char *p) {
    return SDL_strrchr(p, '/');
}
#endif


/* Interfaces for the various music interfaces, ordered by priority */
static Mix_MusicInterface *s_music_interfaces[] =
{
    &Mix_MusicInterface_WAV,
    &Mix_MusicInterface_OGG,
};

extern "C" int get_num_music_interfaces(void)
{
    return SDL_arraysize(s_music_interfaces);
}

extern "C" Mix_MusicInterface *get_music_interface(int index)
{
    return s_music_interfaces[index];
}

extern "C" int Mix_GetNumMusicDecoders(void)
{
    return num_decoders;
}

extern "C" const char *Mix_GetMusicDecoder(int index)
{
    if ((index < 0) || (index >= num_decoders)) {
        return NULL;
    }
    return music_decoders[index];
}

extern "C" SDL_bool Mix_HasMusicDecoder(const char *name)
{
    int index;
    for (index = 0; index < num_decoders; ++index) {
        if (SDL_strcasecmp(name, music_decoders[index]) == 0) {
                return SDL_TRUE;
        }
    }
    return SDL_FALSE;
}

static void add_music_decoder(const char *decoder)
{
    void *ptr;
    int i;

    /* Check to see if we already have this decoder */
    for (i = 0; i < num_decoders; ++i) {
        if (SDL_strcmp(music_decoders[i], decoder) == 0) {
            return;
        }
    }

    ptr = SDL_realloc((void *)music_decoders, ((size_t)num_decoders + 1) * sizeof(const char *));
    if (ptr == NULL) {
        return;  /* oh well, go on without it. */
    }
    music_decoders = (const char **) ptr;
    music_decoders[num_decoders++] = decoder;
}

/* Local low-level functions prototypes */
static void music_internal_initialize_volume(Mix_Music * volatile &music_playing);
static void music_internal_volume(Mix_Music * volatile &music_playing, int volume);
static int  music_internal_play(Mix_Music * music, int channel, int play_count, double position);
static int  music_internal_position(Mix_Music * volatile &music_playing, double position);
static bool music_internal_playing(Mix_Music * volatile &music_playing);
static void music_internal_halt(Mix_Music * volatile &music_playing);


/* Support for hooking when the music has finished */
static Mix_MusicFinishedCallback music_finished_hook = NULL;

extern "C" void Mix_HookMusicFinished(Mix_MusicFinishedCallback music_finished)
{
    Mix_LockAudio();
    music_finished_hook = music_finished;
    Mix_UnlockAudio();
}

/* Convenience function to fill audio and mix at the specified volume
   This is called from many music player's GetAudio callback.
 */
extern "C" int music_pcm_getaudio(void *context, void *data, int bytes, int volume,
                       int (*GetSome)(void *context, void *data, int bytes, SDL_bool *done))
{
    Uint8 *snd = (Uint8 *)data;
    Uint8 *dst;
    int len = bytes;
    int zero_cycles = 0;
    const int MAX_ZERO_CYCLES = 10; /* just try to catch infinite loops */
    SDL_bool done = SDL_FALSE;

    if (volume == MIX_MAX_VOLUME) {
        dst = snd;
    } else {
        dst = SDL_stack_alloc(Uint8, (size_t)bytes);
    }
    while (len > 0 && !done) {
        int consumed = GetSome(context, dst, len, &done);
        if (consumed < 0) {
            break;
        }
        if (consumed == 0) {
            ++zero_cycles;
            if (zero_cycles > MAX_ZERO_CYCLES) {
                /* We went too many cycles with no data, we're done */
                done = SDL_TRUE;
            }
            continue;
        }
        zero_cycles = 0;

        if (volume == MIX_MAX_VOLUME) {
            dst += consumed;
        } else {
            SDL_MixAudioFormat(snd, dst, music_spec.format, (Uint32)consumed, volume);
            snd += consumed;
        }
        len -= consumed;
    }
    if (volume != MIX_MAX_VOLUME) {
        SDL_stack_free(dst);
    }
    return len;
}

extern "C" void SDLCALL music_mixer_single(int channel_id, music_channel_t * channel, Uint8 *stream, int len)
{
    SDL_bool done = SDL_FALSE;
    
    Mix_Music * volatile &music_playing = channel->music_playing;
    
    int cur_volume = (music_volume * channel->channel_volume) / MIX_MAX_VOLUME;
    
    while (music_playing && !music_playing->paused && music_active && len > 0 && !done) {
        /* Handle fading */
        if (music_playing->fading != MIX_NO_FADING) {
            if (music_playing->fade_step++ < music_playing->fade_steps) {
                int volume;
                int fade_step = music_playing->fade_step;
                int fade_steps = music_playing->fade_steps;

                if (music_playing->fading == MIX_FADING_OUT) {
                    volume = (cur_volume * (fade_steps-fade_step)) / fade_steps;
                } else { /* Fading in */
                    volume = (cur_volume * fade_step) / fade_steps;
                }
                music_internal_volume(music_playing, volume);
            } else {
                if (music_playing->fading == MIX_FADING_OUT) {
                    music_internal_halt(music_playing);
                    if (music_finished_hook) {
                        music_finished_hook(channel_id);
                    }
                    return;
                }
                music_playing->fading = MIX_NO_FADING;
                music_internal_volume(music_playing, cur_volume);
            }
        }
        else
        {
            music_internal_volume(music_playing, cur_volume);
        }

        if (music_playing->interface->GetAudio) {
            int left = music_playing->interface->GetAudio(music_playing->context, stream, len);
            if (left != 0) {
                /* Either an error or finished playing with data left */
                music_playing->playing = SDL_FALSE;
                done = SDL_TRUE;
            }
            if (left > 0) {
                stream += (len - left);
                len = left;
            } else {
                len = 0;
            }
        } else {
            len = 0;
        }

        if (!music_internal_playing(music_playing)) {
            music_internal_halt(music_playing);
            if (music_finished_hook) {
                music_finished_hook(channel_id);
            }
        }
    }
}


/* Mixing function */
extern "C" void SDLCALL music_mixer(void *, Uint8 *stream, int len)
{
    music_mixer_single(0, &music_channels[0], stream, len);
}

extern "C" void pause_async_music(int pause_on)
{
    if (!music_active)
    {
        return;
    }
    
    for(int i = 0; i < NUM_MUSIC_CHANNELS; i++)
    {
        if (!music_channels[i].music_playing || !music_channels[i].music_playing->interface) {
            continue;
        }

        if (pause_on) {
            if (music_channels[i].music_playing->interface->Pause) {
                music_channels[i].music_playing->interface->Pause(music_channels[i].music_playing->context);
            }
        } else {
            if (music_channels[i].music_playing->interface->Resume) {
                music_channels[i].music_playing->interface->Resume(music_channels[i].music_playing->context);
            }
        }
    }
}

/* Load the music interface libraries for a given music type */
extern "C" SDL_bool load_music_type(Mix_MusicType type)
{
    size_t i;
    int loaded = 0;
    for (i = 0; i < SDL_arraysize(s_music_interfaces); ++i) {
        Mix_MusicInterface *interface = s_music_interfaces[i];
        if (interface->type != type) {
            continue;
        }
        if (!interface->loaded) {
            char hint[64];
            SDL_snprintf(hint, sizeof(hint), "SDL_MIXER_DISABLE_%s", interface->tag);
            if (SDL_GetHintBoolean(hint, SDL_FALSE)) {
                continue;
            }

            if (interface->Load && interface->Load() < 0) {
                if (SDL_GetHintBoolean(SDL_MIXER_HINT_DEBUG_MUSIC_INTERFACES, SDL_FALSE)) {
                    SDL_Log("Couldn't load %s: %s\n", interface->tag, Mix_GetError());
                }
                continue;
            }
            interface->loaded = SDL_TRUE;
        }
        ++loaded;
    }
    return (loaded > 0) ? SDL_TRUE : SDL_FALSE;
}

/* Open the music interfaces for a given music type */
extern "C" SDL_bool open_music_type(Mix_MusicType type)
{
    size_t i;
    int opened = 0;

    if (!music_spec.format) {
        /* Music isn't opened yet */
        return SDL_FALSE;
    }

    for (i = 0; i < SDL_arraysize(s_music_interfaces); ++i) {
        Mix_MusicInterface *interface = s_music_interfaces[i];
        if (!interface->loaded) {
            continue;
        }
        if (type != MUS_NONE && interface->type != type) {
            continue;
        }
        
        if (!interface->opened) {
            if (interface->Open && interface->Open(&music_spec) < 0) {
                if (SDL_GetHintBoolean(SDL_MIXER_HINT_DEBUG_MUSIC_INTERFACES, SDL_FALSE)) {
                    SDL_Log("Couldn't open %s: %s\n", interface->tag, Mix_GetError());
                }
                continue;
            }
            interface->opened = SDL_TRUE;
            add_music_decoder(interface->tag);
        }
        ++opened;
    }

    if (has_music(MUS_OGG)) {
        add_music_decoder("OGG");
        add_chunk_decoder("OGG");
    }

    return (opened > 0) ? SDL_TRUE : SDL_FALSE;
}

/* Initialize the music interfaces with a certain desired audio format */
extern "C" void open_music(const SDL_AudioSpec *spec)
{
    /* Load the music interfaces that don't have explicit initialization */
    load_music_type(MUS_WAV);

    /* Open all the interfaces that are loaded */
    music_spec = *spec;
    open_music_type(MUS_NONE);

    Mix_VolumeMusic(MIX_MAX_VOLUME);

    /* Calculate the number of ms for each callback */
    ms_per_step = (int) (((float)spec->samples * 1000.0f) / spec->freq);
}

/* Return SDL_TRUE if the music type is available */
extern "C" SDL_bool has_music(Mix_MusicType type)
{
    size_t i;
    for (i = 0; i < SDL_arraysize(s_music_interfaces); ++i) {
        Mix_MusicInterface *interface = s_music_interfaces[i];
        if (interface->type != type) {
            continue;
        }
        if (interface->opened) {
            return SDL_TRUE;
        }
    }
    return SDL_FALSE;
}

extern "C" Mix_MusicType detect_music_type(SDL_RWops *src)
{
    Uint8 magic[12];

    if (SDL_RWread(src, magic, 1, 12) != 12) {
        Mix_SetError("Couldn't read first 12 bytes of audio data");
        return MUS_NONE;
    }
    SDL_RWseek(src, -12, RW_SEEK_CUR);

    /* WAVE files have the magic four bytes "RIFF"
       AIFF files have the magic 12 bytes "FORM" XXXX "AIFF" */
    if (((SDL_memcmp(magic, "RIFF", 4) == 0) && (SDL_memcmp((magic+8), "WAVE", 4) == 0)) ||
        (SDL_memcmp(magic, "FORM", 4) == 0)) {
        return MUS_WAV;
    }

    /* Ogg Vorbis files have the magic four bytes "OggS" */
    if (SDL_memcmp(magic, "OggS", 4) == 0) {
        SDL_RWseek(src, 28, RW_SEEK_CUR);
        SDL_RWread(src, magic, 1, 8);
        SDL_RWseek(src,-36, RW_SEEK_CUR);
        if (SDL_memcmp(magic, "OpusHead", 8) == 0) {
            Mix_SetError("OPUS codec not supported");
            return MUS_NONE;
        }
        if (magic[0] == 0x7F && SDL_memcmp(magic + 1, "FLAC", 4) == 0) {
            Mix_SetError("FLAC codec not supported");
            return MUS_NONE;
        }
        return MUS_OGG;
    }

    /* FLAC files have the magic four bytes "fLaC" */
    if (SDL_memcmp(magic, "fLaC", 4) == 0) {
        Mix_SetError("FLAC codec not supported");
        return MUS_NONE;
    }

    /* WavPack files have the magic four bytes "wvpk" */
    if (SDL_memcmp(magic, "wvpk", 4) == 0) {
        Mix_SetError("WAVPACK codec not supported");
        return MUS_NONE;
    }

    /* MIDI files have the magic four bytes "MThd" */
    if (SDL_memcmp(magic, "MThd", 4) == 0) {
        Mix_SetError("MIDI not supported");
        return MUS_NONE;
    }

    /* RIFF MIDI files have the magic four bytes "RIFF" and then "RMID" */
    if ((SDL_memcmp(magic, "RIFF", 4) == 0) && (SDL_memcmp(magic + 8, "RMID", 4) == 0)) {
        Mix_SetError("MIDI not supported");
        return MUS_NONE;
    }

    if (SDL_memcmp(magic, "ID3", 3) == 0 ||
    /* see: https://bugzilla.libsdl.org/show_bug.cgi?id=5322 */
        (magic[0] == 0xFF && (magic[1] & 0xE6) == 0xE2)) {
        Mix_SetError("MP3 not supported");
        return MUS_NONE;
    }
    
    Mix_SetError("unknown codec");
    return MUS_NONE;
}

/* Load a music file */
extern "C" Mix_Music *Mix_LoadMUS(const char *file)
{
    size_t i;
    void *context;
    char *ext;
    Mix_MusicType type = MUS_NONE;
    SDL_RWops *src;

    for (i = 0; i < SDL_arraysize(s_music_interfaces); ++i) {
        Mix_MusicInterface *interface = s_music_interfaces[i];
        if (!interface->opened || !interface->CreateFromFile) {
            continue;
        }

        context = interface->CreateFromFile(file);
        if (context) {
            const char *p;
            /* Allocate memory for the music structure */
            Mix_Music *music = (Mix_Music *)SDL_calloc(1, sizeof(Mix_Music));
            if (music == NULL) {
                Mix_OutOfMemory();
                return NULL;
            }
            music->interface = interface;
            music->context = context;
            p = get_last_dirsep(file);
            SDL_strlcpy(music->filename, (p != NULL)? p + 1 : file, 1024);
            return music;
        }
    }

    src = SDL_RWFromFile(file, "rb");
    if (src == NULL) {
        Mix_SetError("Couldn't open '%s'", file);
        return NULL;
    }

    /* Use the extension as a first guess on the file type */
    type = MUS_NONE;
    ext = SDL_strrchr(file, '.');
    if (ext) {
        ++ext; /* skip the dot in the extension */
        if (SDL_strcasecmp(ext, "WAV") == 0) {
            type = MUS_WAV;
        } else if (SDL_strcasecmp(ext, "OGG") == 0) {
            type = MUS_OGG;
        }
    }
    return Mix_LoadMUSType_RW(src, type, SDL_TRUE);
}

extern "C" Mix_Music *Mix_LoadMUS_RW(SDL_RWops *src, int freesrc)
{
    return Mix_LoadMUSType_RW(src, MUS_NONE, freesrc);
}

extern "C" Mix_Music *Mix_LoadMUSType_RW(SDL_RWops *src, Mix_MusicType type, int freesrc)
{
    size_t i;
    void *context;
    Sint64 start;

    if (!src) {
        Mix_SetError("RWops pointer is NULL");
        return NULL;
    }
    start = SDL_RWtell(src);

    /* If the caller wants auto-detection, figure out what kind of file
     * this is. */
    if (type == MUS_NONE) {
        if ((type = detect_music_type(src)) == MUS_NONE) {
            /* Don't call Mix_SetError() since detect_music_type() does that. */
            if (freesrc) {
                SDL_RWclose(src);
            }
            return NULL;
        }
    }

    Mix_ClearError();

    if (load_music_type(type) && open_music_type(type)) {
        for (i = 0; i < SDL_arraysize(s_music_interfaces); ++i) {
            Mix_MusicInterface *interface = s_music_interfaces[i];
            if (!interface->opened || type != interface->type || !interface->CreateFromRW) {
                continue;
            }

            context = interface->CreateFromRW(src, freesrc);
            if (context) {
                /* Allocate memory for the music structure */
                Mix_Music *music = (Mix_Music *)SDL_calloc(1, sizeof(Mix_Music));
                if (music == NULL) {
                    interface->Delete(context);
                    Mix_OutOfMemory();
                    return NULL;
                }
                music->interface = interface;
                music->context = context;

                if (SDL_GetHintBoolean(SDL_MIXER_HINT_DEBUG_MUSIC_INTERFACES, SDL_FALSE)) {
                    SDL_Log("Loaded music with %s\n", interface->tag);
                }
                return music;
            }

            /* Reset the stream for the next decoder */
            SDL_RWseek(src, start, RW_SEEK_SET);
        }
    }

    if (!*Mix_GetError()) {
        Mix_SetError("Unrecognized audio format");
    }
    if (freesrc) {
        SDL_RWclose(src);
    } else {
        SDL_RWseek(src, start, RW_SEEK_SET);
    }
    return NULL;
}

/* Free a music chunk previously loaded */
extern "C" void Mix_FreeMusic(Mix_Music *music)
{
    if (music) {
        /* Stop the music if it's currently playing */
        Mix_LockAudio();
        
        for(int i = 0; i < NUM_MUSIC_CHANNELS; i++)
        {
            if (music == music_channels[i].music_playing)
            {
                /* Wait for any fade out to finish */
                while (music_active && music->fading == MIX_FADING_OUT)
                {
                    Mix_UnlockAudio();
                    SDL_Delay(100);
                    Mix_LockAudio();
                }
                if (music == music_channels[i].music_playing)
                {
                    music_internal_halt(music);
                }
            }
        }
        Mix_UnlockAudio();

        music->interface->Delete(music->context);
        SDL_free(music);
    }
}

/* Find out the music format of a mixer music, or the currently playing
   music, if 'music' is NULL.
*/
extern "C" Mix_MusicType Mix_GetMusicType(const Mix_Music *music)
{
    if (music) {
        return music->interface->type;
    } else {
        return MUS_NONE;
    }
}

extern "C" Mix_MusicType Mix_GetMusicChannelType(int channel)
{
    if(channel < 0 || channel >= NUM_MUSIC_CHANNELS) return MUS_NONE;
    Mix_MusicType type = MUS_NONE;
    Mix_LockAudio();
    if(music_channels[channel].music_playing)
    {
        type = music_channels[channel].music_playing->interface->type;
    }
    Mix_UnlockAudio();
    return type;
}

/* Play a music chunk.  Returns 0, or -1 if there was an error.
 */
static int music_internal_play(Mix_Music *music, int channel, int play_count, double position)
{
    if(channel < 0 || channel >= NUM_MUSIC_CHANNELS || music->playing) return -1;
    
    int retval = 0;

    /* Note the music we're playing */
    if (music_channels[channel].music_playing) {
        music_internal_halt(music_channels[channel].music_playing);
    }
    
    music_channels[channel].music_playing = music;
    music_channels[channel].channel_volume = MIX_MAX_VOLUME;
    music->playing = SDL_TRUE;
    music->paused = SDL_FALSE;

    /* Set the initial volume */
    music_internal_initialize_volume(music);

    /* Set up for playback */
    retval = music->interface->Play(music->context, play_count);

    /* Set the playback position, note any errors if an offset is used */
    if (retval == 0) {
        if (position > 0.0) {
            if (music_internal_position(music, position) < 0) {
                Mix_SetError("Position not implemented for music type");
                retval = -1;
            }
        } else {
            music_internal_position(music, 0.0);
        }
    }

    /* If the setup failed, we're not playing any music anymore */
    if (retval < 0) {
        music->playing = SDL_FALSE;
        music_channels[channel].music_playing = NULL;
    }
    return retval;
}

extern "C" int Mix_FadeInMusicPos(Mix_Music *music, int channel, int loops, int ms, double position)
{
    if(channel < 0 || channel >= NUM_MUSIC_CHANNELS) return -1;
    
    int retval;

    if (ms_per_step == 0) {
        return Mix_SetError("Audio device hasn't been opened");
    }

    /* Don't play null pointers :-) */
    if (music == NULL) {
        return Mix_SetError("music parameter was NULL");
    }

    /* Setup the data */
    if (ms) {
        music->fading = MIX_FADING_IN;
    } else {
        music->fading = MIX_NO_FADING;
    }
    music->fade_step = 0;
    music->fade_steps = (ms + ms_per_step - 1) / ms_per_step;

    /* Play the puppy */
    Mix_LockAudio();
    /* If the current music is fading out, wait for the fade to complete */
    while (music_channels[channel].music_playing && (music_channels[channel].music_playing->fading == MIX_FADING_OUT)) {
        Mix_UnlockAudio();
        SDL_Delay(100);
        Mix_LockAudio();
    }
    if (loops == 0) {
        /* Loop is the number of times to play the audio */
        loops = 1;
    }
    retval = music_internal_play(music, channel, loops, position);
    /* Set music as active */
    music_active = (retval == 0);
    Mix_UnlockAudio();

    return retval;
}

extern "C" int Mix_FadeInMusic(Mix_Music *music, int channel, int loops, int ms)
{
    return Mix_FadeInMusicPos(music, channel, loops, ms, 0.0);
}

extern "C" int Mix_PlayMusic(Mix_Music *music, int channel, int loops)
{
    return Mix_FadeInMusicPos(music, channel, loops, 0, 0.0);
}

/* Set the playing music position */
static int music_internal_position(Mix_Music * volatile &music_playing, double position)
{
    if (music_playing->interface->Seek) {
        return music_playing->interface->Seek(music_playing->context, position);
    }
    return -1;
}

extern "C" int Mix_SetMusicPosition(int channel, double position)
{
    if(channel < 0 || channel >= NUM_MUSIC_CHANNELS) return -1;
    
    int retval;

    Mix_LockAudio();
    if (music_channels[channel].music_playing) {
        retval = music_internal_position(music_channels[channel].music_playing, position);
        if (retval < 0) {
            Mix_SetError("Position not implemented for music type");
        }
    } else {
        Mix_SetError("Music isn't playing");
        retval = -1;
    }
    Mix_UnlockAudio();

    return retval;
}

/* Set the playing music position */
static double music_internal_position_get(Mix_Music *music)
{
    if (music->interface->Tell) {
        return music->interface->Tell(music->context);
    }
    return -1;
}

extern "C" double Mix_GetMusicPosition(int channel)
{
    if(channel < 0 || channel >= NUM_MUSIC_CHANNELS) return -1;
    
    double retval;

    Mix_LockAudio();
    if (music_channels[channel].music_playing) {
        retval = music_internal_position_get(music_channels[channel].music_playing);
    } else {
        Mix_SetError("Music isn't playing");
        retval = -1.0;
    }
    Mix_UnlockAudio();

    return retval;
}

static double music_internal_duration(Mix_Music *music)
{
    if (music->interface->Duration) {
        return music->interface->Duration(music->context);
    } else {
        Mix_SetError("Duration not implemented for music type");
        return -1;
    }
}

extern "C" double Mix_MusicDuration(Mix_Music *music)
{
    double retval;

    Mix_LockAudio();
    if (music) {
        retval = music_internal_duration(music);
    } else {
        Mix_SetError("music is NULL");
        retval = -1.0;
    }
    Mix_UnlockAudio();

    return retval;
}

extern "C" double Mix_MusicChannelDuration(int channel)
{
    if(channel < 0 || channel >= NUM_MUSIC_CHANNELS) return -1;
    
    double retval;

    Mix_LockAudio();
    if (music_channels[channel].music_playing) {
        retval = music_internal_duration(music_channels[channel].music_playing);
    } else {
        Mix_SetError("Music isn't playing");
        retval = -1.0;
    }
    Mix_UnlockAudio();

    return retval;
}

/* Set the music's initial volume */
static void music_internal_initialize_volume(Mix_Music * volatile &music_playing)
{
    if (music_playing->fading == MIX_FADING_IN) {
        music_internal_volume(music_playing, 0);
    } else {
        music_internal_volume(music_playing, music_volume);
    }
}

/* Set the music volume */
static void music_internal_volume(Mix_Music * volatile &music_playing, int volume)
{
    if (music_playing->interface->SetVolume) {
        music_playing->interface->SetVolume(music_playing->context, volume);
    }
}

extern "C" int Mix_VolumeMusic(int volume)
{
    int prev_volume;

    prev_volume = music_volume;
    if (volume < 0) {
        return prev_volume;
    }
    if (volume > SDL_MIX_MAXVOLUME) {
        volume = SDL_MIX_MAXVOLUME;
    }
    music_volume = volume;
    return prev_volume;
}

extern "C" int Mix_GetMusicVolume()
{
    return music_volume;
}

extern "C" int Mix_GetMusicChannelVolume(int channel)
{
    if(channel < 0 || channel >= NUM_MUSIC_CHANNELS) return -1;
    return music_channels[channel].channel_volume;
}

extern "C" int Mix_SetMusicChannelVolume(int channel, int volume)
{
    if(channel < 0 || channel >= NUM_MUSIC_CHANNELS) return -1;
    
    int prev_volume = music_channels[channel].channel_volume;
    
    if (volume < 0) {
        return prev_volume;
    }
    if (volume > SDL_MIX_MAXVOLUME) {
        volume = SDL_MIX_MAXVOLUME;
    }
    music_channels[channel].channel_volume = volume;
    return prev_volume;
}


/* Halt playing of music */
static void music_internal_halt(Mix_Music * volatile &music_playing)
{
    if (music_playing->interface->Stop) {
        music_playing->interface->Stop(music_playing->context);
    }

    music_playing->playing = SDL_FALSE;
    music_playing->fading = MIX_NO_FADING;
    music_playing = NULL;
}

extern "C" int Mix_HaltMusic(void)
{
    Mix_LockAudio();
    for(int i = 0; i < NUM_MUSIC_CHANNELS; i++)
    {
        if (music_channels[i].music_playing)
        {
            music_internal_halt(music_channels[i].music_playing);
            if (music_finished_hook)
            {
                music_finished_hook(i);
            }
        }
    }
    Mix_UnlockAudio();

    return 0;
}

int Mix_HaltMusicChannel(int channel)
{
    if(channel < 0 || channel >= NUM_MUSIC_CHANNELS) return -1;
    
    Mix_LockAudio();
    if (music_channels[channel].music_playing)
    {
        music_internal_halt(music_channels[channel].music_playing);
        if (music_finished_hook)
        {
            music_finished_hook(channel);
        }
    }
    Mix_UnlockAudio();

    return 0;
}


static int Mix_FadeOutMusicChannelInternal(int ms, int channel)
{
    int retval = 0;
    Mix_Music * volatile music_playing = music_channels[channel].music_playing;
    
    if (music_playing) {
        int fade_steps = (ms + ms_per_step - 1) / ms_per_step;
        if (music_playing->fading == MIX_NO_FADING) {
            music_playing->fade_step = 0;
        } else {
            int step;
            int old_fade_steps = music_playing->fade_steps;
            if (music_playing->fading == MIX_FADING_OUT) {
                step = music_playing->fade_step;
            } else {
                step = old_fade_steps - music_playing->fade_step + 1;
            }
            music_playing->fade_step = (step * fade_steps) / old_fade_steps;
        }
        music_playing->fading = MIX_FADING_OUT;
        music_playing->fade_steps = fade_steps;
        retval = 1;
    }
    return retval;
}

int Mix_FadeOutMusicChannel(int channel, int ms)
{
    if(channel < 0 || channel >= NUM_MUSIC_CHANNELS) return 0;

    if (ms_per_step == 0)
    {
        Mix_SetError("Audio device hasn't been opened");
        return 0;
    }

    if (ms <= 0) {  /* just halt immediately. */
        Mix_HaltMusicChannel(channel);
        return 1;
    }

    Mix_LockAudio();
    
    int retval = Mix_FadeOutMusicChannelInternal(ms, channel);
    
    Mix_UnlockAudio();

    return retval;
}

/* Progressively stop the music */
int Mix_FadeOutMusic(int ms)
{
    if (ms_per_step == 0) {
        Mix_SetError("Audio device hasn't been opened");
        return 0;
    }

    if (ms <= 0) {  /* just halt immediately. */
        Mix_HaltMusic();
        return 1;
    }
    
    Mix_LockAudio();
    
    int retval = 0;
    
    for(int i = 0; i < NUM_MUSIC_CHANNELS; i++)
    {
        retval |= Mix_FadeOutMusicChannelInternal(ms, i);
    }
    
    Mix_UnlockAudio();

    return retval;
}

Mix_Fading Mix_FadingMusic(int channel)
{
    Mix_Fading fading = MIX_NO_FADING;

    Mix_LockAudio();
    if (music_channels[channel].music_playing) {
        fading = music_channels[channel].music_playing->fading;
    }
    Mix_UnlockAudio();

    return fading;
}

/* Pause/Resume the music stream */
void Mix_PauseMusic(void)
{
    Mix_LockAudio();
    
    for(int i = 0; i < NUM_MUSIC_CHANNELS; i++)
    {
        if (music_channels[i].music_playing && !music_channels[i].music_playing->paused)
        {
            if (music_channels[i].music_playing->interface->Pause)
            {
                music_channels[i].music_playing->interface->Pause(music_channels[i].music_playing->context);
            }
            music_channels[i].music_playing->paused = true;
        }
    }
    music_active = SDL_FALSE;
    Mix_UnlockAudio();
}

void Mix_PauseMusicChannel(int channel)
{
    if(channel < 0 || channel >= NUM_MUSIC_CHANNELS) return;
    
    Mix_LockAudio();
    if (music_channels[channel].music_playing && !music_channels[channel].music_playing->paused)
    {
        if (music_channels[channel].music_playing->interface->Pause)
        {
            music_channels[channel].music_playing->interface->Pause(music_channels[channel].music_playing->context);
        }
        music_channels[channel].music_playing->paused = true;
    }
    Mix_UnlockAudio();
}

void Mix_ResumeMusic(void)
{
    Mix_LockAudio();
    for(int i = 0; i < NUM_MUSIC_CHANNELS; i++)
    {
        if (music_channels[i].music_playing && music_channels[i].music_playing->paused)
        {
            if (music_channels[i].music_playing->interface->Resume)
            {
                music_channels[i].music_playing->interface->Resume(music_channels[i].music_playing->context);
            }
            music_channels[i].music_playing->paused = false;
        }
    }
    music_active = SDL_TRUE;
    Mix_UnlockAudio();
}

void Mix_ResumeMusicChannel(int channel)
{
    if(channel < 0 || channel >= NUM_MUSIC_CHANNELS) return;
    
    Mix_LockAudio();
    if (music_channels[channel].music_playing && music_channels[channel].music_playing->paused)
    {
        if (music_channels[channel].music_playing->interface->Resume)
        {
            music_channels[channel].music_playing->interface->Resume(music_channels[channel].music_playing->context);
        }
        music_channels[channel].music_playing->paused = false;
    }
    Mix_UnlockAudio();
}

void Mix_RewindMusic(int channel)
{
    Mix_SetMusicPosition(channel, 0.0);
}

int Mix_PausedMusic(void)
{
    return (music_active == SDL_FALSE);
}

/* Check the status of the music */
static bool music_internal_playing(Mix_Music * volatile &music_playing)
{
    if (!music_playing) {
        return SDL_FALSE;
    }

    if (music_playing->interface->IsPlaying) {
        music_playing->playing = music_playing->interface->IsPlaying(music_playing->context);
    }
    return music_playing->playing;
}

int Mix_PlayingMusic(void)
{
    bool playing;

    Mix_LockAudio();
    
    for(int i = 0; i < NUM_MUSIC_CHANNELS; i++)
    {
        playing = music_internal_playing(music_channels[i].music_playing) || playing;
    }
    Mix_UnlockAudio();

    return playing ? 1 : 0;
}

/* Uninitialize the music interfaces */
void close_music(void)
{
    size_t i;

    Mix_HaltMusic();

    for (i = 0; i < SDL_arraysize(s_music_interfaces); ++i) {
        Mix_MusicInterface *interface = s_music_interfaces[i];
        if (!interface || !interface->opened) {
            continue;
        }

        if (interface->Close) {
            interface->Close();
        }
        interface->opened = SDL_FALSE;
    }

    /* rcg06042009 report available decoders at runtime. */
    if (music_decoders) {
        SDL_free((void *)music_decoders);
        music_decoders = NULL;
    }
    num_decoders = 0;

    ms_per_step = 0;
}

/* Unload the music interface libraries */
void unload_music(void)
{
    size_t i;
    for (i = 0; i < SDL_arraysize(s_music_interfaces); ++i) {
        Mix_MusicInterface *interface = s_music_interfaces[i];
        if (!interface || !interface->loaded) {
            continue;
        }

        if (interface->Unload) {
            interface->Unload();
        }
        interface->loaded = SDL_FALSE;
    }
}

/* vi: set ts=4 sw=4 expandtab: */
